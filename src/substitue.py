...
def substitute_texte(input_texte: str | List[str], subs_file: str) -> str:
    """
    ...
    :param subs_file: Le chemin du fichier qui contient les mots à remplacer et leurs substituts.
    ...
    subs_df = pd.read_csv(
        subs_file, sep="\t", header=None, names=["token", "substitute"]
    )
    ...
    for i, mot in enumerate(input_texte):
        substitute = subs_df.loc[subs_df["token"] == mot, "substitute"]
        # Si le mot a un substitut on le change, sinon on le laisse
        input_texte[i] = substitute.iloc[0] if not substitute.empty else mot
